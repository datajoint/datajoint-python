...
class QueryExpression:
    ...
    A QueryExpression object has a source, a restriction (an AndList), _proj_attrs (a list), and an _proj_map (a dict).
    ...
    Then projection is applied by selecting the attributes from _proj_attrs and creating new attributes from _proj_map.

    Application of operators does not always lead to the creation of a subquery.
    A subquery is generated when:
        1. A restriction is applied on attributes remapped in _proj_map.
        2. A projection is applied remapping attributes already remapped in _proj_map
    ...
    def heading(self):
        ...
        proj and other operators may modify.
        """
        if isinstance(self.source, QueryExpression):
            if self._heading is None:
                self._heading = self.source.heading
        else:
            # in dj.Table's subclasses self._heading is already defined
    ...
    def _make_condition(self, arg):
        ...
        """
        return make_condition(self, arg)
