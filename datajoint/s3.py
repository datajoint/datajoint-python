...
class Folder:
    ...
    def __init__(self, endpoint, bucket, access_key, secret_key, location, **_):
        ...
        self.root_path = location.lstrip('/')
    ...
    def get(self, relative_name):
        try:
            return self.client.get_object(self.bucket, '/'.join((self.root_path, relative_name))).data
    ...
    def fget(self, relative_name, local_filepath):
        ...
        name = '/'.join((self.root_path, relative_name))
    ...
    def get_size(self, relative_name):
        try:
            return self.client.stat_object(self.bucket, '/'.join((self.root_path, relative_name))).size
        except minio.error.NoSuchKey:
            raise errors.MissingExternalFile from None

    def list_objects(self, folder=''):
        return self.client.list_objects(self.bucket, '/'.join((self.root_path, folder, '')), recursive=True)
