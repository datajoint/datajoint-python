...
class Base(Table, metaclass=abc.ABCMeta):
    ...
    def add_attribute(self, definition, after=None):
        ...
        sql = self._field_to_SQL(_parse_attr_def(definition))
    ...
    def alter_attribute(self, attr_name, new_definition):
        ...
        """
        sql = self._field_to_SQL(_parse_attr_def(new_definition))
    ...
    def _parse_declaration(self):
        ...
        for line in declaration[1:]:
            if line.startswith('---'):
                ...
                index_defs.append(_parse_index_def(line))
            elif fieldP.match(line):
                field_defs.append(_parse_attr_def(line, in_key))
            else:
                raise DataJointError(
...
def _parse_index_def(line):    # why is this a method of Base instead of a local function?
    """
