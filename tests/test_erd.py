...
class TestERD:
    ...
    def test_dependencies():
        ...
        deps.load()
        assert_true(
            all(cls.full_table_name in deps for cls in (A, B, B.C, D, E, E.F, L))
        )
        ...
        assert_true(
            set(deps.descendants(L.full_table_name)).issubset(
    ...
    def test_erd_algebra():
        ...
        assert_true(
            erd1.nodes_to_show == set(cls.full_table_name for cls in (B, B.C, E, E.F))
        )
        assert_true(
            erd2.nodes_to_show == set(cls.full_table_name for cls in (A, B, D, E, L))
        )
        assert_true(erd3.nodes_to_show == set(cls.full_table_name for cls in (B, E)))
        assert_true(
            erd4.nodes_to_show == set(cls.full_table_name for cls in (B.C, E.F))
        )
